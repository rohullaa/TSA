...
from models.NorBert import BERT
...
def train():
    ...
    for epoch in range(args.EPOCHS):
        ...
        for sentences, lengths, labels, char_list in train_iter:
            ...
            loss.backward() # takes long due to torch version
            optimizer.step()
            scheduler.step()
...
def compute_score(sents, golds, preds):
    ...
    propor_f1 = proportional_analysis(flat_golds, flat_preds, args.ENCODING)
    return binary_f1, propor_f1
...
if __name__ == '__main__':
    ...
    parser.add_argument("--FREEZE", "-freeze", action='store_true')
    parser.add_argument("--MODEL_PATH", "-path", default="../bert/216/") #   "/cluster/shared/nlpl/data/vectors/latest/216/"
    ...
    parser.add_argument("--RUN_NAME", "-rn", default='transformer_add_char', type=str)
    parser.add_argument("--ENCODING", "-en", default='BIOUL', type=str)
    ...
    if args.ENCODING == 'BIO':
        alphabet = Alphabet("data/train.conll", "data/dev.conll", "data/test.conll", args.ENCODING)
        train_data = BERT_DATA(data_file="data/train.conll", alphabet=alphabet, encoding=args.ENCODING)
        dev_data = BERT_DATA(data_file="data/dev.conll", alphabet=alphabet, encoding=args.ENCODING, vocab=train_data.vocab)
        test_data = BERT_DATA(data_file="data/test.conll", alphabet=alphabet, encoding=args.ENCODING, vocab=train_data.vocab)
    elif args.ENCODING == 'BIOUL':
        alphabet = Alphabet("data/train_bioul.conll", "data/dev_bioul.conll", "data/test_bioul.conll", args.ENCODING)
        train_data = BERT_DATA(data_file="data/train_bioul.conll", alphabet=alphabet, encoding=args.ENCODING)
        dev_data = BERT_DATA(data_file="data/dev_bioul.conll", alphabet=alphabet, encoding=args.ENCODING,
                             vocab=train_data.vocab)
        test_data = BERT_DATA(data_file="data/test_bioul.conll", alphabet=alphabet, encoding=args.ENCODING,
                              vocab=train_data.vocab)
