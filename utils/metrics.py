...
def binary_analysis(pred_analysis):
    ...
    logger.info("Target prec: {0:.3f}".format(prec))
    logger.info("Target recall: {0:.3f}".format(rec))
    logger.info("Target F1: {0:.3f}".format(f1))
    logger.info('')
    return f1


def proportional_analysis(flat_gold_labels, flat_predictions, encoding):
    if encoding == 'BIOUL':
    ...
    logger.info("Proportional results:")
    logger.info("#" * 80)
    logger.info('')
